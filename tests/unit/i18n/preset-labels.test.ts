...
describe('preset labels', () => {
  describe('built-in presets', () => {
    ...
    const t =
      (service: I18nService) =>
      (key: Parameters<I18nService['t']>[0]): string =>
        service.t(key);
    ...
    it.each(DEFAULT_FORMAT_PRESETS.map(preset => [preset.id, preset] as const))(
      ...
      (id, preset) => {
        [en, fr].forEach(service => {
          const name = presetName(preset, t(service));
          const desc = presetDescription(preset, t(service));
    ...
    it('resolves the name from the id, not from stored data', () => {
      ...
      expect(presetName(iso, t(fr))).toBe('ISO 8601');
      expect(presetDescription(iso, t(fr))).toBe('Format ISO standard');
    });
    ...
    it('ignores a name left over in stored data by an earlier version', () => {
      ...
      expect(presetName(stale, t(fr))).toBe('ISO 8601');
      expect(presetDescription(stale, t(fr))).toBe('Format ISO standard');
    });
    ...
    it('reads the French label of a preset whose English label is different', () => {
      ...
      expect(presetName(short, t(en))).toBe('Locale short');
      expect(presetName(short, t(fr))).toBe('Date courte');
    });
  });

  describe('user-defined presets', () => {
    const fr = new I18nService('fr');
    const t = (key: Parameters<I18nService['t']>[0]): string => fr.t(key);
